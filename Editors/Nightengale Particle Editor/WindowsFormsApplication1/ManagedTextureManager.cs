...
namespace SGP
{
    public sealed class ManagedTextureManager
    {
        ...
        public bool Draw(int id, int x, int y,
                         ...
                         int rotx, int roty, float rot, System.Drawing.Color color)
        {
            // Make sure the id is in range.
            Debug.Assert((id > -1 && id < textures.Count), "id is out of range");
            ...
            // Draw the sprite.
            sprite.Draw(((TEXTURE)textures[id]).texture, section, Vector3.Empty, Vector3.Empty, color);
                         ...
                         int rotx, int roty, float rot, int color)
        {
            ...
            // Draw the sprite.
            if (color != 0)
                return Draw(id,x,y,scalex,scaley,section,rotx,roty,rot,System.Drawing.Color.FromArgb(color));
            else
                return Draw(id, x, y, scalex, scaley, section, rotx, roty, rot, System.Drawing.Color.White);
        }
        ...
        /// <param name="roty">Y position on the texture to rotate around.</param>
        /// <param name="rot">Amount to rotate by (in radians).</param>
        /// <param name="color">Alpha value for the color to modulate the image with.</param>
        /// <param name="color">Red value for the color to modulate the image with.</param>
        /// <param name="color">Green value for the color to modulate the image with.</param>
        /// <param name="color">Blue value for the color to modulate the image with.</param>
        /// <returns>true if successful, false otherwise.</returns>
        public bool Draw(int id, int x, int y,
                         float scalex, float scaley, System.Drawing.Rectangle section,
                         int rotx, int roty, float rot, int alpha, int red, int green, int blue)
        {
            // success.
            // Draw the sprite.
            return Draw(id, x, y, scalex, scaley, section, rotx, roty, rot, System.Drawing.Color.FromArgb(alpha,red,green,blue));
        }
